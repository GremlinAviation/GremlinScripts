...
Evac.zones = {
    ...
    evac = {
        ...
        create = function(_zone, _smoke, _side)
            return Evac._internal.zones.create(_zone, _smoke, _side, Evac._internal.modes.EVAC)
        end,
        -- Activate a zone for evacuation operations
        activate = function(_zone)
            return Evac._internal.zones.activate(_zone, Evac._internal.modes.EVAC)
        end,
        -- Manually override the remaining units in a zone
        setRemaining = function(_zone, _numberOrComposition)
            return Evac._internal.zones.setRemaining(_zone, _numberOrComposition, Evac._internal.modes.EVAC)
        end,
        -- Count the number of evacuees in a zone
        count = function(_zone)
            return Evac._internal.zones.count(_zone, Evac._internal.modes.EVAC)
        end,
        -- Check whether a given unit is in an evac zone
        isIn = function(_unit)
            return Evac._internal.zones.isIn(_unit, Evac._internal.modes.EVAC)
        end,
        -- Deactivate a zone for evacuation operations
        deactivate = function(_zone)
            return Evac._internal.zones.deactivate(_zone, Evac._internal.modes.EVAC)
        end,
        -- Forget a zone once it should no longer be used as part of the Evac ecosystem
        remove = function(_zone)
            return Evac._internal.zones.remove(_zone, Evac._internal.modes.EVAC)
        end,
    },
    -- Relay Zone methods
    relay = {
        ...
        create = function(_zone, _smoke, _side)
            return Evac._internal.zones.create(_zone, _smoke, _side, Evac._internal.modes.RELAY)
        end,
        -- Activate a zone for evacuation operations
        activate = function(_zone)
            return Evac._internal.zones.activate(_zone, Evac._internal.modes.RELAY)
        end,
        -- Manually override the remaining units in a zone
        setRemaining = function(_zone, _numberOrComposition)
            return Evac._internal.zones.setRemaining(_zone, _numberOrComposition, Evac._internal.modes.RELAY)
        end,
        -- Count the number of evacuees in a zone
        count = function(_zone)
            return Evac._internal.zones.count(_zone, Evac._internal.modes.RELAY)
        end,
        -- Check whether a given unit is in a relay zone
        isIn = function(_unit)
            return Evac._internal.zones.isIn(_unit, Evac._internal.modes.RELAY)
        end,
        -- Deactivate a zone for evacuation operations
        deactivate = function(_zone)
            return Evac._internal.zones.deactivate(_zone, Evac._internal.modes.RELAY)
        end,
        -- Forget a zone once it should no longer be used as part of the Evac ecosystem
        remove = function(_zone)
            return Evac._internal.zones.remove(_zone, Evac._internal.modes.RELAY)
        end,
    },
    -- Safe Zone methods
    safe = {
        ...
        create = function(_zone, _smoke, _side)
            return Evac._internal.zones.create(_zone, _smoke, _side, Evac._internal.modes.SAFE)
        end,
        -- Activate a zone for evacuation operations
        activate = function(_zone)
            return Evac._internal.zones.activate(_zone, Evac._internal.modes.SAFE)
        end,
        -- Count the number of evacuees in a zone
        count = function(_zone)
            return Evac._internal.zones.count(_zone, Evac._internal.modes.SAFE)
        end,
        -- Check whether a given unit is in a safe zone
        isIn = function(_unit)
            return Evac._internal.zones.isIn(_unit, Evac._internal.modes.SAFE)
        end,
        -- Deactivate a zone for evacuation operations
        deactivate = function(_zone)
            return Evac._internal.zones.deactivate(_zone, Evac._internal.modes.SAFE)
        end,
        -- Forget a zone once it should no longer be used as part of the Evac ecosystem
        remove = function(_zone)
            return Evac._internal.zones.remove(_zone, Evac._internal.modes.SAFE)
        end,
    },
}
    --- Unit methods
    ...
    inZone = function(_unit, _evacMode)
        ...
        if _unitObj ~= nil then
            if type(_evacMode) == "string" then
                ...
                if _evacMode == Evac._internal.modes.EVAC then
                    for _, _zoneData in pairs(Evac._state.zones.evac) do
                        if _zoneData.active and mist.pointInZone(_unitObj:getPoint(), _zoneData.name) then
                            return true
                        end
                    end
                elseif _evacMode == Evac._internal.modes.RELAY then
                    for _, _zoneData in pairs(Evac._state.zones.relay) do
                        if _zoneData.active and mist.pointInZone(_unitObj:getPoint(), _zoneData.name) then
                            return true
                        end
                    end
                elseif _evacMode == Evac._internal.modes.SAFE then
                    for _, _zoneData in pairs(Evac._state.zones.safe) do
                        if _zoneData.active and mist.pointInZone(_unitObj:getPoint(), _zoneData.name) then
                            return true
                        end
        ...
        mist.scheduleFunction(function(_start)
            ...
            if _left <= 0 then
                ...
                for _, _evacuee in pairs(Evac._state.extractionUnits[_unit]) do
                    ...
                    _groups[Unit:getByName(_evacuee.name).getGroup().name] = true
                end
...
Evac._internal.beacons = {
    spawn = function(_zone, _side, _country, _batteryLife, _name)
        ...
        local _beaconDetails = {
            ...
            uhf = _freq.uhf,
            fm = _freq.fm,
            group = mist.dynAdd(_radioGroup).name,
            text = _freqsText,
            battery = _battery,
            side = _side,
        }
    ...
    list = function(_unit)
        ...
        if _unit ~= nil then
            ...
            if _message ~= "" then
                Gremlin.displayMessageTo(mist.DBs.unitsByName[_unit]:getGroup(), "Radio Beacons:\n" .. _message, 20)
            else
                Gremlin.displayMessageTo(mist.DBs.unitsByName[_unit]:getGroup(), "No Active Radio Beacons", 20)
            end
        end
    end,
...
Evac._internal.zones = {
    create = function(_zone, _smoke, _side, _evacMode)
        Evac._state.zones[Evac._internal.modeToText[_evacMode]][_zone] = {
            active = false,
            name = _zone,
            side = _side,
            smoke = _smoke,
    ...
    generateEvacuees = function(_side, _numberOrComposition, _country)
        ...
        if type(_numberOrComposition) == "number" then
            ...
            _composition = _numberOrComposition
        end

        for _i, _unit in pairs(_composition) do
    ...
    activate = function(_zone, _evacMode)
        Evac._state.zones[Evac._internal.modeToText[_evacMode]][_zone].active = true
    end,
    setRemaining = function(_zone, _side, _country, _numberOrComposition, _evacMode)
    ...
    count = function(_zone, _evacMode)
        ...
        if (_evacMode == Evac._internal.modes.EVAC and Evac._state.zones.evac[_zone] ~= nil)
            or (_evacMode == Evac._internal.modes.RELAY and Evac._state.zones.relay[_zone] ~= nil)
            or (_evacMode == Evac._internal.modes.SAFE and Evac._state.zones.safe[_zone] ~= nil)
        then
            _count = #Evac._state.extractableNow[_zone]
        end

        return _count
    end,
    isIn = function(_unit, _evacMode)
        return Evac._internal.aircraft.inZone(_unit, _evacMode)
    end,
    deactivate = function(_zone, _evacMode)
        Evac._state.zones[Evac._internal.modeToText[_evacMode]][_zone].active = false
    end,
    remove = function(_zone, _evacMode)
        for _unit, _ in pairs(Evac._state.extractableNow[_zone]) do
            Unit:getByName(_unit).destroy()
        end

        Evac._state.extractableNow[_zone] = nil
        Evac._state.zones[Evac._internal.modeToText[_evacMode]][_zone] = nil
    end,
}
...
Evac._internal.menu = {
    addToF10 = function()
        ...
        for _, _unit in pairs(Evac._state.extractionUnits) do
...
Evac._internal.utils = {
    ...
    randomizeWeight = function(_weight)
        return (math.random(90, 120) * Evac.spawnWeight) / 100
    end,
...
`Evac._internal.modes`), `name`, `smoke` (one of the constants in
    ...
    if config ~= nil then
        ...
        if config.startingZones ~= nil then
            for _, _zone in pairs(config.startingZones) do
                if _zone.mode == Evac._internal.modes.EVAC then
                    Evac.zones.evac.create(_zone.name, _zone.smoke, _zone.side)
                elseif _zone.mode == Evac._internal.modes.RELAY then
                    Evac.zones.relay.create(_zone.name, _zone.smoke, _zone.side)
                elseif _zone.mode == Evac._internal.modes.SAFE then
