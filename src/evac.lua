    ...
    unitDataToList = function(_side, _units, _point, _scatterRadius)
        ...
        local typeTranslation = {
            [0] = {
                ...
                M249 = 'soldier_m249',
                RPG = 'Paratrooper with RPG-16',
                StingerIgla = 'soldier_igla',
                ['2B11'] = '2B11',
                JTAC = 'JTAC',
            },
            [1] = {
            ...
            [2] = {
                ['Carrier Seaman'] = 'Carrier Seaman',
                Infantry = 'Infantry M4',
                M249 = 'soldier_m249',
                RPG = 'soldier_rpg',
                StingerIgla = 'soldier_stinger',
                ['2B11'] = '2B11',
                JTAC = 'JTAC',
            },
        }
...
Evac._internal.doSpawns = function()
    ...
    local _remaining = function(_side)
        Gremlin.log.debug(Evac.Id, string.format('Checking Whether We Can Spawn More Evacuees : %s side has %s out of %s already spawned', Gremlin.SideToText[_side], Gremlin.utils.inspect(Evac._state.spawns.alreadySpawned[_side]), Gremlin.utils.inspect(Evac.maxExtractable[_side])))
        ...
        for _side, _rateSource in pairs(_rates) do
            ...
            if _lastChecked ~= nil and _lastChecked + (math.abs(_rate.per) * _rate.period) <= timer.getTime() then
                ...
                if _rate.units == 0 then
                    _units = {}
                    for _type, _allowed in pairs(Evac.maxExtractable[_side]) do
                        for i = 1, _allowed do
                ...
                Gremlin.log.debug(Evac.Id, string.format('Evacuee Count Updated : now at %s out of %s', Gremlin.utils.inspect(Evac._state.spawns.alreadySpawned[_side]), Gremlin.utils.inspect(Evac.maxExtractable[_side])))
            end
        end
...
function Evac:setup(config)
    ...
    if not Evac._state.alreadyInitialized or config.forceReload then
        ...
        Gremlin.log.debug(Evac.Id, string.format('Configuration Loaded : %s', Gremlin.utils.inspect({
